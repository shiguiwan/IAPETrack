...
class AttentionOU(nn.Module):
    ...
    def forward(self, x, template_mask, search_feat_len, token_len, tgt_type=None, attn_masking=True, threshold=0., ratio=0.):
        ...
        if self.divide:
            ...
            q_o, k, v = qkv[0], qkv[1], qkv[2]  # B,12,320,64 Make torchscript happy (cannot use tensor as tuple)
            # indice = torch.arange(template_feat_len * 2 + token_len,
            #                        template_feat_len * 2 + token_len + search_feat_len, 1).cuda()
            # indices = torch.cat([torch.IntTensor(0).zero_().cuda(), indice], dim=0)
            # q = q_o.index_select(2, indices)
            q = torch.cat([q_o[:,:,0].unsqueeze(2), q_o[:,:,template_feat_len + token_len:]], dim=2)
            ...
            attn = attn.softmax(dim=-1) #B,12,321,385
            # if torch.isnan(attn).any():
            #     print('attn:', attn)
...
class FSOU2Block(nn.Module):
    ...
    def forward(self, x_m1, x_m2, template_mask, box, feature_1, feature_2,  search_feat_len, token_len, threshold, tgt_type):
        if self.fs_keep != -1:
    ...
    def sample_target_batch(self, im_feature, target_bb, search_area_factor, output_sz):
        ...
        im_crop_padded_list = torch.stack(im_crop_padded_list).squeeze(0)
        #resize_factor_list = torch.stack(resize_factor_list)
        att_mask_list = torch.stack(att_mask_list).squeeze(0)
...
class VisionTransformerFSOU2(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        #self.cls_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        # self.track_query_rgb = nn.Parameter(torch.zeros(5, 1, embed_dim))  ###batch_size
        # self.track_query_t = nn.Parameter(torch.zeros(5, 1, embed_dim))
        ...
        self.prefeature_rgb_max = {}
        self.prefeature_t_max = {}
        self.prefeature_rgb_lowmax = {}
        self.prefeature_t_lowmax = {}
        #for i in range(fs_index):
        for i in range(depth):
            self.prefeature_rgb[i] = None
            self.prefeature_t[i] = None
            self.prefeature_rgb_max[i] = None
            self.prefeature_t_max[i] = None
        self.search_box_prv = {}
        self.search_box_max = {}
        self.search_box_lowmax = {}
        #self.count = 0
        #score
        self.score_max = 0
        self.score = 0
        ...
        #     self.save_prefeatrue_rgb = {}
        #     self.save_prefeature_t = {}
        #     for i in range(depth):
        #         self.save_prefeature_rgb[i] = None
        #         self.save_prefeature_t[i] = None

    def forward_features(self, z_rgb, z_t, x_rgb, x_t, template_mask_rgb, template_mask_t, search_box,
                         search_feat_len, threshold, tgt_type, i, score,
        ...
        #训练中track_query不清零
        B = x_rgb.shape[0]
        # B_token = self.track_query_rgb.shape[0]
        # if B == B_token:
        #     track_query_rgb = self.track_query_rgb
        #     track_query_t = self.track_query_t
        # else:
        #     track_query_rgb = self.track_query_rgb[:B]
        #     track_query_t = self.track_query_t[:B]
        # if self.track_query_rgb is None:
        #     track_query_rgb = self.track_query_save_rgb[0]
        #     track_query_t = self.track_query_save_t[0]
        # else:
        #     track_query_rgb = self.track_query_rgb
        #     track_query_t = self.track_query_t ######track_query改
        track_query_rgb = self.track_query_rgb
        track_query_t = self.track_query_t
        ...
        box_change = False
        max_change = False
        # if score > 0.75 or search_box[0] is None:
        ...
        #     self.score_max = 0
        #     self.search_box_max[0] = None
        #     self.search_box_max[1] = None
        # else:
        #     self.count += 1
        #     #self.score_list.append(score)
        #     if score > self.score_max:
        #         self.score_max = score
        #         self.search_box_max[0] = search_box[0]
        #         self.search_box_max[1] = search_box[1]
        #         max_change = True
        #     if self.count > 100:
        #         search_box_feature[0] = self.search_box_max[0]
        #         search_box_feature[1] = self.search_box_max[1]
        #         #self.count = 0
        #         #self.score_list = None
        #         box_change = True
        #     else:
        #         search_box_feature[0] = self.search_box_prv[0]
        #         search_box_feature[1] = self.search_box_prv[1]
        if search_box[0] is None:
            search_box_feature[0] = search_box[0]
            search_box_feature[1] = search_box[1]
            self.search_box_prv[0] = search_box[0]
            self.search_box_prv[1] = search_box[1]
        #score
        elif score > 0.79: #0.83:
            if score > self.score:
                self.search_box_max[0] = search_box[0]
                self.search_box_max[1] = search_box[1]
            if self.count >= 20: #20:
                search_box_feature[0] = self.search_box_max[0]
                search_box_feature[1] = self.search_box_max[1]
                self.search_box_prv[0] = self.search_box_max[0]
                self.search_box_prv[1] = self.search_box_max[1]
                self.search_box_max[0] = None
                self.search_box_max[1] = None
            else:
                search_box_feature[0] = self.search_box_prv[0]
            ...
            if score > self.score_max:
                self.score_max = score
                self.search_box_lowmax[0] = search_box[0]
                self.search_box_lowmax[1] = search_box[1]
                max_change = True
            if self.count > 100:
                search_box_feature[0] = self.search_box_lowmax[0]
                search_box_feature[1] = self.search_box_lowmax[1]
                box_change = True
            else:
                search_box_feature[0] = self.search_box_prv[0]
                search_box_feature[1] = self.search_box_prv[1]
        ...
        for i, blk in enumerate(self.blocks):
            ...
            #     self.prefeature_rgb_max[index] = None
            #     self.prefeature_t_max[index] = None
            # else:
            #     if max_change:
            #         self.prefeature_rgb_max[index] = self.prefeature_rgb_new[index]
            #         self.prefeature_t_max[index] = self.prefeature_t_new[index]
            #     if box_change:
            #         feature_rgb = self.prefeature_rgb_max[index]
            #         feature_t = self.prefeature_t_max[index]
            #     else:
            ...
            if score > 0.79: #0.83:
                ...
                if index == 11:
                    self.count += 1
                if max_change:
                    self.prefeature_rgb_lowmax[index] = self.prefeature_rgb_new[index]
                    self.prefeature_t_lowmax[index] = self.prefeature_t_new[index]
                if box_change:
                    feature_rgb = self.prefeature_rgb_lowmax[index]
                    feature_t = self.prefeature_t_lowmax[index]
                else:
                    feature_rgb = self.prefeature_rgb[index]
                    feature_t = self.prefeature_t[index]
            #mse_loss
        ...
        # newweight_track_query_rgb = (x_rgb[:, :token_len, :].clone()).detach().cuda()
        # newweight_track_query_t = (x_t[:, :token_len, :].clone()).detach().cuda()
        # self.track_query_rgb.weight = nn.Parameter(newweight_track_query_rgb)
        # self.track_query_t.weight = nn.Parameter(newweight_track_query_t)
        self.track_query_rgb = (x_rgb[:, :token_len, :].clone()).detach().cuda() ######track_query改
        self.track_query_t = (x_t[:, :token_len, :].clone()).detach().cuda()
        # self.track_query_save_rgb.weight = nn.Parameter(self.track_query_rgb)
        # self.track_query_save_t.weight = nn.Parameter(self.track_query_t)
        x = x_rgb + x_t
    ...
    def forward(self, z_rgb, z_t, x_rgb, x_t, template_mask_rgb, template_mask_t, search_box, search_feat_len, threshold, tgt_type, i, score,
                #track_query,
                #masks_fusion_rgb, masks_fusion_t,
                **kwargs):

        x, decisions = self.forward_features(z_rgb, z_t, x_rgb, x_t, template_mask_rgb, template_mask_t, search_box,
                                             search_feat_len, threshold, tgt_type, i, score #track_query
                                             #masks_fusion_rgb, masks_fusion_t
                                             )
        return x, decisions
